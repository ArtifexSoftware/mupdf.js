...
class DisplayList extends Userdata {
	...
	toStructuredText(options = "") {
		...
		return new StructuredText(libmupdf._wasm_new_stext_page_from_display_list(this, STRING(options)))
	}
...
class Page extends Userdata {
	...
	toStructuredText(options = "") {
		checkType(options, "string")
		return new StructuredText(libmupdf._wasm_new_stext_page_from_page(this, STRING(options)))
	}
...
class PDFDocument extends Document {
	...
	addStream(buf, obj) {
		obj = PDFOBJ(this, obj)
		buf = toBuffer(buf)
		return fromPDFObject(this, libmupdf._wasm_pdf_add_stream(this, buf, obj.pointer, 0))
	}

	addRawStream(buf, obj) {
		obj = PDFOBJ(this, obj)
		buf = toBuffer(buf)
		return fromPDFObject(this, libmupdf._wasm_pdf_add_stream(this, buf, obj.pointer, 1))
	}

	newGraftMap() {
		return new PDFGraftMap(this, libmupdf._wasm_pdf_new_graft_map(this))
	}

	graftObject(obj) {
		checkType(obj, PDFObject)
		return fromPDFObject(this, libmupdf._wasm_pdf_graft_object(this, obj))
	}
	...
	graftPage(to, srcDoc, srcPage) {
		...
		libmupdf._wasm_pdf_graft_page(this, to, srcDoc, srcPage)
	}
...
class PDFPage extends Page {
	...
	getObject() {
		return keepPDFObject(this._doc, libmupdf._wasm_pdf_page_get_obj(this))
	}
	...
	setPageBox(which, rect) {
		which = toEnum(which, PDFPage.BOXES)
		checkRect(rect)
		libmupdf._wasm_pdf_set_page_box(this, which, RECT(rect))
	}
...
class PDFObject extends Userdata {
	...
	asNumber() { return libmupdf._wasm_pdf_to_real(this) }
	...
	resolve() {
		return fromPDFObject(this._doc, libmupdf._wasm_pdf_resolve_indirect(this))
	}
	...
	get(...path) { return fromPDFObject(this._doc, this._get(path)) }
	...
	getInheritable(key) {
		if (typeof key === PDFObject)
			return fromPDFObject(this._doc, libmupdf._wasm_pdf_dict_get_inheritable(this, key.pointer))
		return fromPDFObject(this._doc, libmupdf._wasm_pdf_dict_gets_inheritable(this, STRING(key)))
	}
	...
	forEach(fn) {
		if (this.isArray()) {
			...
			for (let i = 0; i < n; ++i) {
				let key = fromPDFObject(this._doc, libmupdf._wasm_pdf_dict_get_key(this.pointer, i))
				let val = fromPDFObject(this._doc, libmupdf._wasm_pdf_dict_get_val(this.pointer, i))
				fn(val, key, this)
			}
		}
	}
...
class PDFGraftMap extends Userdata {
	static _drop = "_wasm_pdf_drop_graft_map"
	...
	graftObject(obj) {
		checkType(obj, PDFObject)
		return fromPDFObject(this._doc, libmupdf._wasm_pdf_graft_mapped_object(this, obj))
	}
	...
	graftPage(to, srcDoc, srcPage) {
		...
		checkType(srcDoc, PDFDocument)
		checkType(srcPage, "number")
		libmupdf._wasm_pdf_graft_mapped_page(this, to, srcDoc, srcPage)
	}
}
...
class PDFAnnotation extends Userdata {
	...
	getHiddenForEditing() {
		return libmupdf._wasm_pdf_annot_hidden_for_editing(this)
	}

	setHiddenForEditing(isHidden) {
		libmupdf._wasm_pdf_set_annot_hidden_for_editing(this, isHidden)
	}
