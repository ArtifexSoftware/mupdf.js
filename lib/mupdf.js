...
class Userdata {
	...
	destroy() {
		this.constructor._finalizer.unregister(this.pointer)
...
class Image extends Userdata {
	...
	constructor(arg1, arg2) {
		...
		else if (arg1 instanceof Pixmap && arg2 instanceof ColorSpace)
	...
	getColorSpace() {
		...
		if (cs)
			return new ColorSpace(libmupdf._wasm_keep_colorspace(cs.pointer))
		return null
	}
...
class StrokeState extends Userdata {
	...
	getLineWidth(w) {
		libmupdf._wasm_stroke_state_get_linewidth(this.pointer, w)
	}
	getMiterLimit() {
...
class Pixmap extends Userdata {
	...
	constructor(arg1, bbox = null, alpha = false) {
		...
		if (arg1 === null) {
			checkRect(bbox)
			pointer = libmupdf._wasm_new_pixmap_with_bbox(0, RECT(bbox), alpha)
		}
		super(pointer)
	}
...
class DisplayListDevice extends Device {
	constructor(displayList) {
		...
		super(
			libmupdf._wasm_new_display_list_device(displayList.pointer)
		)
	}
}
...
class Document extends Userdata {
	...
	hasPermission(flag) {
		...
		return libmupdf._wasm_has_permission(this.pointer, Document.PERMISSION[flag])
	}
...
class OutlineIterator extends Userdata {
	...
	insert(item) {
		return libmupdf._wasm_outline_insert(this.pointer, STRING_OPT(item.title), STRING2_OPT(item.uri), item.open)
	}

	update(item) {
		libmupdf._wasm_outline_update(this.pointer, STRING_OPT(item.title), STRING2_OPT(item.uri), item.open)
	}
}
...
class PDFDocument extends Document {
	...
	newBool(v) { return this._fromPDFObjectNew(libmupdf._wasm_pdf_new_bool(v)) }
	...
	addFont(font) {
		checkType(font, Font)
		return this._fromPDFObjectNew(libmupdf._wasm_pdf_add_font(this.pointer, font.pointer))
	}
